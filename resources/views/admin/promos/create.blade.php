            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                        ...
                        placeholder="Contoh: Flash Sale November">
                    @error('name')
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        placeholder="Deskripsi singkat tentang promo ini">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="flash_sale" {{ old('type') == 'flash_sale' ? 'selected' : '' }}>🔥 Flash Sale</option>
                        <option value="seasonal" {{ old('type') == 'seasonal' ? 'selected' : '' }}>🎄 Seasonal</option>
                        <option value="clearance" {{ old('type') == 'clearance' ? 'selected' : '' }}>📦 Clearance</option>
                ...
                <div>
                    <label for="banner_image" class="block text-sm font-medium text-gray-700 mb-1">Banner (Opsional)</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="max_discount" id="max_discount" value="{{ old('max_discount') }}" step="0.01" min="0"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        placeholder="50000">
                    <p class="mt-1 text-xs text-gray-500">Kosongkan jika tidak ada batas maksimal</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="apply_to_all_books" id="apply_to_all_books" value="1" {{ old('apply_to_all_books') ? 'checked' : '' }}
            ...
            <div id="book-selection" class="{{ old('apply_to_all_books') ? 'hidden' : '' }}">
